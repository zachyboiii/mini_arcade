...
def paddle_a_up():
    y = paddle_a.ycor()
    y += paddle_a_speed
    if y + 50 < 250:  # Check if the new y-coordinate is within the screen limits
        paddle_a.sety(y)
...
def paddle_b_down():
    y = paddle_b.ycor()
    y -= paddle_b_speed
    if y - 50 > -235:  # Check if the new y-coordinate is within the screen limits
        paddle_b.sety(y)
...
# Keep the window open to show final score until closed manually
turtle.done()